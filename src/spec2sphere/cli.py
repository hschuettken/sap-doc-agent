...
Usage:
    python -m spec2sphere.cli --config config.yaml [--scan] [--sync] [--qa] [--report] [--all]
    python -m spec2sphere.cli --config config.yaml --all   # Run full pipeline
"""
...
from spec2sphere.agents.doc_sync import DocSyncAgent
from spec2sphere.agents.doc_qa import DocQAAgent, load_standard
from spec2sphere.agents.code_quality import CodeQualityAgent
from spec2sphere.agents.brs_traceability import BRSTraceabilityAgent, TraceReport
...
async def run_audit(args) -> None:
    ...
    if args.config:
        try:
            from spec2sphere.config import load_config
