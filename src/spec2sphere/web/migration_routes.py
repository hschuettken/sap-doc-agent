...
def create_migration_api_router(output_dir: Path) -> APIRouter:
    ...
    async def trigger_interpret(project_id: str):
        ...
        for chain_file in chain_files:
            try:
                from spec2sphere.tasks.migration_tasks import interpret_chain_task
    ...
    async def trigger_classify(project_id: str):
        ...
        for intent_file in sorted(chains_dir.glob("*_intent.json")):
            ...
            if not chain_file.exists():
                continue
            try:
                from spec2sphere.tasks.migration_tasks import classify_chain_task
    ...
    async def trigger_design(project_id: str):
        ...
        for classified_file in sorted(chains_dir.glob("*_classified.json")):
            ...
            if not chain_file.exists():
                continue
            try:
                from spec2sphere.tasks.migration_tasks import design_target_task
    ...
    async def trigger_generate(project_id: str):
        ...
        for target_file in sorted(chains_dir.glob("*_target.json")):
            ...
            try:
                from spec2sphere.tasks.migration_tasks import generate_sql_task
    ...
    async def get_report(project_id: str):
        ...
        from spec2sphere.migration.effort import estimate_chain_effort
        from spec2sphere.migration.models import ClassifiedChain, TargetArchitecture, ViewSpec
            ...
            for chain_file in sorted(chains_dir.glob("*.json")):
                ...
                try:
                    ...
                    if classified_file.exists():
                        ...
                        from spec2sphere.migration.models import IntentCard, MigrationClassification
