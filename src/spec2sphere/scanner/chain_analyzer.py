...
async def analyze_chain_steps(chain: DataFlowChain, llm: LLMProvider) -> DataFlowChain:
    ...
    for i, step in enumerate(result.steps):
        ...
        data = await llm.generate_json(
            prompt, schema=_STEP_SCHEMA, system=_STEP_SYSTEM, tier="chain_analyzer", data_in_context=True
        )
        if data and isinstance(data, dict):
            ...
            text = await llm.generate(prompt, system=_STEP_SYSTEM, tier="chain_analyzer", data_in_context=True)
            if text:
...
async def summarize_chain(chain: DataFlowChain, llm: LLMProvider) -> DataFlowChain:
    ...
    data = await llm.generate_json(prompt, schema=_CHAIN_SCHEMA, system=_CHAIN_SYSTEM, tier="chain_analyzer", data_in_context=True)
    if data:
